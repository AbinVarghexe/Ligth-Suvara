...
class _VideoResourcesScreenState extends State<VideoResourcesScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final isDynamicSection = widget.initialSectionId != null;

    return PopScope(
      canPop: isDynamicSection || _selectedClass == null,
      onPopInvokedWithResult: (didPop, result) {
        if (!didPop) {
          if (!isDynamicSection && _selectedClass != null) {
            _backToGrid();
          }
        }
      },
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            isDynamicSection
                ? (widget.initialSectionTitle ?? 'Resources')
                : (_selectedClass == null
                    ? 'Video Resources'
        ...
        body: FadeTransition(
          ...
          child: AnimatedSwitcher(
            ...
            child: (isDynamicSection || _selectedClass != null)
  ...
  Widget _buildClassResourcesView(Color navyColor) {
    final keyString = widget.initialSectionId != null 
        ? 'resources_view_${widget.initialSectionId}' 
        : 'resources_view_$_selectedClass';
    return Container(
  ...
  Widget _buildResourcesList(Color navyColor) {
    final isDynamic = widget.initialSectionId != null;
    final activeColors = isDynamic 
        ? [widget.initialSectionColor ?? navyColor, widget.initialSectionColor?.withValues(alpha: 0.8) ?? navyColor]
        : _classConfig[_selectedClass! - 1].colors;
    final activeIcon = isDynamic 
        ? Icons.category_rounded 
        : _classConfig[_selectedClass! - 1].icon;
    final activeTagline = isDynamic 
        ? 'Explore resources and documents' 
        : _classConfig[_selectedClass! - 1].tagline;
    final activeTitle = isDynamic 
        ? (widget.initialSectionTitle ?? 'Resources') 
        : 'Class $_selectedClass';
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final keyString = isDynamic ? 'resources_scroll_${widget.initialSectionId}' : 'resources_scroll_$_selectedClass';
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate((context, index) {
                  ...
                  return _AnimatedChapterCard(
                    ...
                    resources: resources,
                    navyColor: navyColor,
                    accentColor: activeColors.first,

...
Future<void> openYamaprarthanakalApp(BuildContext context) async {
  ...
  try {
    ...
    debugPrint(
      'Installation check result: ${isInstalled == true
          ? "INSTALLED"
          : isInstalled == false
          ? "NOT INSTALLED"
          : "UNKNOWN"}',
  ...
  if (isInstalled == false) {
    ...
    debugPrint(
      '? Installation status unknown. Launch was attempted - assuming it worked if app is installed.',
    );
  }
  ...
  if (isInstalled == false) {
    ...
    try {
      ...
      if (await canLaunchUrl(uri)) {
        ...
        // if (context.mounted) {
        //   ScaffoldMessenger.of(context).showSnackBar(
        //     const SnackBar(content: Text("Could not open Play Store."), backgroundColor: Colors.orange),
        //   );
        // }
      }
    } catch (playStoreError) {
